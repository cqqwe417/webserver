...
//    17、公司网站、18、助手、19、派报、20、厨窗、21、陌拜、22、红布条、23、Salescall、24、老客户推荐 -->
		...
		<div style="width: 30%; height: 90%; float: left;">
			 	...
			 	<hr style="color:black;background-color:black"/>
			 	 ...
			 	 <div id="contactgrid" class="mini-datagrid"  showPager="false" style="width:100%;height: 40%;margin-left: 20px;" url="houseResource/sell/houseContactData.do?houseId=${houseId }">
	                    <div property="columns">
	                        ...
	                        <div field="hostName" width="50">房东姓名</div>
	                        <div field="hostMobile" width="100" allowSort="true" align="center"
			...
			<div  style="width: 100%; height: 55%;">
			 	...
			 	<hr style="color:black;background-color:black"/>
			 	 <div id="yjgsr" class="mini-datagrid" style="width:100%;height:55%;margin-left: 20px;"                     
...
<script type="text/javascript">
	...
	grid.on("drawcell", function(e) {
			...
			if (column.name == "action") {
				...
				if(record.pub!=1&&record.publishagent!=0){
					html = lc.strFormat('<a class="mini-button" style="margin-left:5px;padding-left:10px;padding-right:10px;" onclick="edit(\'{0}\',{1})">编辑</a><a class="mini-button" style="margin-left:5px;padding-left:10px;padding-right:10px;" onclick="deleteAgent(\'{0}\')">删除</a>',flag,record.publishagent);
				}else if(record.pub==1&&record.publishagent!=0){
					html=lc.strFormat('<a class="mini-button" style="margin-left:5px;padding-left:10px;padding-right:10px;" onclick="edit(\'{0}\',{1})">编辑</a>',flag,record.publishagent);
				}else if(record.pub!=1&&record.publishagent==0){
					html=lc.strFormat('<a class="mini-button" style="margin-left:5px;padding-left:10px;padding-right:10px;" onclick="edit(\'{0}\',{1})">增加</a>',flag,record.publishagent);
				}
				e.cellHtml = html;
	...
	contactgrid.on("drawcell", function(e) {
	 ...
	 function deleteAgent(flag) {
         mini.confirm("确定删除记录？", "提示",
             function (action) {
                 if (action == "ok") {
                     $.ajax({
 						url: "houseResource/sell/deleteAgent.do?houseId=${houseId}&houseAgentId=0&flag="+flag,
         				type: 'post',
                        cache: false,
	...
	function edit(flag,storeid) {
        mini.open({
            url: "houseResource/sell/editPage.do?houseId=${houseId}&flag="+flag+"&storeId="+storeid,
            title: "编辑",
            width: 400, 
            height:400,
	...
	function add() {
        mini.open({
            url: "houseResource/sell/addContactPage.do?houseId=${houseId}",
            title: "增加",
            width: 400, 
            height:400,
            ondestroy: function (action) {
